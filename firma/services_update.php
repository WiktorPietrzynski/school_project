...
$stmt = $connection->prepare("SELECT * FROM services INNER JOIN workers ON worker_id = service_worker INNER JOIN contractors ON contractor_id = service_contractor WHERE service_id=? and worker_user_id = ?;");
$stmt->bind_param('ii', $_GET['id'], $_SESSION['user_id']);
$stmt->execute();
$service = $stmt->get_result();
$count = $service->num_rows;
if ($count == 0) {
    header("location: services.php");
    exit();
}
...
<main class="main">
    <section class="section">
        <header class="section__header">
            <h1 class="section__title">Edycja usługi.</h1>
        </header>
        <div class="section__content section__content--column">
            <?php require_once "include/form_message.php" ?>
            <form class="form--double" action="php/update/service_updating.php?id=<?php echo $_GET['id'] ?>" method="post" id="form_service">
                <div class="form__row">
                            ...
                            if (isset($_GET['branch']) && $_GET['branch'] > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    if (isset($_SESSION['form_worker']) && $_SESSION['form_worker'] == $row['worker_id']) {
                                        echo "<option class='form__option'  selected='selected' value='" . $row['worker_id'] . "' >" . $row['worker_name'] . "</option>";
                                    } else {
                                        if ($service['service_worker'] == $row['worker_id']) {
                                            echo "<option class='form__option'  selected='selected' value='" . $row['worker_id'] . "' >" . $row['worker_name'] . "</option>";
                                        } else {
                        ...
                        <select class="form__select" id="contractor" name="contractor">
                            ...
                            while ($row = $result->fetch_assoc()) {
                                if (isset($_SESSION['form_contractor']) && $_SESSION['form_contractor'] == $row['contractor_id']) {
                                    ...
                                    if ($service['service_contractor'] == $row['contractor_id']) {
                                        echo "<option class='form__option'  selected='selected' value='" . $row['contractor_id'] . "' >" . $row['contractor_first_name'] . " " . $row['contractor_last_name'] . "</option>";
                                    } else {
                ...
                <button class="form__submit" id="button_service" type="button">Edytuj</button>
            </form>
        </div>
    </section>
    <div class="additional-form" id="form_contractor_wrapper">
        <?php require_once "include/form_message.php" ?>
        <form class="form--double" action="php/update/service_updating.php?id=<?php echo $_GET['id'] ?>" id="form_contractor" method="post">
            <div class="form__row">
            ...
            <button class="form__submit" type="submit">Edytuj</button><button class="form__submit" id="button_cancel" type="button">Anuluj</button>
        </form>
    </div>
</main>
<style>
