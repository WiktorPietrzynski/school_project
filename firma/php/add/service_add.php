...
if (!isset($_POST['service_name'])) {
	header('location: ../../service_creator.php');
	exit();
} else {
...
validate_form("../service_creator.php");
if ($contractor != "0") {
...
$stmt = $connection->prepare("INSERT INTO services VALUES (NULL, ?, ?, ?, ?, ? ,?)");
$stmt->bind_param('sssssi', $service_name, $price, $date, $worker, $contractor_id, $_SESSION['user_id']);
if ($stmt->execute()) {
	...
	$_SESSION['form_success_info'] = "Dodano nową usługę.";
	header('location: ../../service_creator.php');
} else {
	$_SESSION['form_valid_info'] = "Błąd dodawania usługi. Spróbuj ponownie później.";
	header("location: ../../service_creator.php");
}
$connection->close();
